...
/**
 * NewFileValidator verifies if an attribute is receiving a valid uploaded file.
 *
...
class NewFileValidator extends Validator
{
    ...
    public $mimeTypes = '';
    /**
     * @var int|\Closure the minimum number of bytes required for the uploaded file.
    ...
    public $minSize;
    /**
     * @var int|\Closure the maximum number of bytes required for the uploaded file.
     * Defaults to null, meaning no limit.
    ...
    {
        if ($this->minSize instanceof \Closure) {
            $this->minSize = call_user_func($this->minSize, $model, $attribute);

        }
        if ($this->maxSize instanceof \Closure) {
            $this->maxSize = call_user_func($this->maxSize, $model, $attribute);
        }
